...
void* AllocateExecutableMemory(size_t size, bool exec)
{
	...
	char *map_hint = 0;
#if defined(_M_X64)
	// Try to request one that is close to our memory location. We use a
	// dummy global variable to give us a good location to start from.
	if (exec && (!map_hint))
		map_hint = (char*)round_page(&hint_location); /* 0.5 GB rounded up to the next page */
#endif
	void* ptr = mmap(map_hint, size, PROT_READ | PROT_WRITE	| PROT_EXEC,
		MAP_ANON | MAP_PRIVATE
#if defined(_M_X64) && defined(MAP_32BIT)
		| (exec && (uintptr_t) map_hint < 0xFFFFFFFF ? MAP_32BIT : 0)
#endif
		, -1, 0);
